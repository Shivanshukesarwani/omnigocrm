...
class ApiClient(private val context:Context){
 ...
 fun logCall(type:String,id:Long,phone:String,duration:Int):Long{return JSONObject(request("POST","calls",JSONObject().put("subject_type",type).put("subject_id",id).put("phone",phone).put("duration_seconds",duration).put("direction","outgoing").put("status","completed").toString())).optLong("id")}
 fun uploadRecording(callId:Long,file:java.io.File){val boundary="----Omni"+System.currentTimeMillis();val c=(URL(base()+"calls/"+callId+"/recording").openConnection() as HttpURLConnection);c.requestMethod="POST";c.doOutput=true;c.setRequestProperty("Authorization","Bearer "+session.token);c.setRequestProperty("Content-Type","multipart/form-data; boundary="+boundary);c.outputStream.use{out->fun w(s:String)=out.write(s.toByteArray());w("--"+boundary+"\\r\\nContent-Disposition: form-data; name=\"recording\"; filename=\""+file.name+"\"\\r\\nContent-Type: audio/mp4\\r\\n\\r\\n");file.inputStream().use{it.copyTo(out)};w("\\r\\n--"+boundary+"--\\r\\n")};c.inputStream.close();c.disconnect()}
