...
class CallRecordingService:Service(){
 ...
 private fun finishCall(){val duration=((System.currentTimeMillis()-started)/1000).toInt();try{recorder?.stop()}catch(_:Exception){file?.delete()};recorder?.release();recorder=null;Thread{try{val api=ApiClient(this);val callId=api.logCall(type,id,phone,duration);if(callId>0&&file?.exists()==true)api.uploadRecording(callId,file!!)}catch(_:Exception){}finally{stopSelf()}}.start()}
